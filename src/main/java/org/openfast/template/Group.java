...
public class Group extends Field {
    ...
    public Group(QName name, Field[] fields, boolean optional) {
        ...
        for (int i = 0; i < fields.length; i++) {
            if (fields[i] instanceof StaticTemplateReference) {
                Field[] referenceFields = ((StaticTemplateReference)fields[i]).getTemplate()
                        .getFields();
        ...
        for (int i = 0; i < fields.length; i++) {
            if (fields[i] instanceof Scalar) {
                if (fields[i].hasChild(FastConstants.LENGTH_FIELD)) {
                    Node lengthNode = (Node)fields[i].getChildren(FastConstants.LENGTH_FIELD)
                            .get(0);
    ...
    public byte[] encode(FieldValue value, Group template, Context context) {
        ...
        BitVectorBuilder presenceMapBuilder = new BitVectorBuilder(groupValue.getGroup()
                .getMaxPresenceMapSize());
        try {
            ...
            for (int fieldIndex = 0; fieldIndex < fields.length; fieldIndex++) {
                ...
                if (!field.isOptional() && fieldValue == null) {
                    Global.handleError(FastConstants.GENERAL_ERROR, "Mandatory field " + field
                            + " is null");
                }
    ...
    protected FieldValue[] decodeFieldValues(InputStream in, Group template, Context context) {
        if (usesPresenceMap()) {
            ...
            if (pmap.isOverlong()) {
                Global.handleError(FastConstants.R7_PMAP_OVERLONG, "The presence map " + pmap
                        + " for the group " + this
                        + " is overlong.");
            }
    ...
    public FieldValue[] decodeFieldValues(InputStream in, Group template,
        ...
        if (pmapReader.hasMoreBitsSet()) {
            Global.handleError(FastConstants.R8_PMAP_TOO_MANY_BITS, "The presence map "
                    + pmapReader + " has too many bits for the group " + this);
        }
        return values;
    }
